...
Scenario.etherToSell = 2.5; // amount of ETH (in ether) to swap for tokens
Scenario.etherTargetLiquidity = 30; //buy tokens only if the the ETH liquidity (in ether) is greater than this
...
Scenario.botContractAddress = '0x0892ecf898130934da6afa65eac5e413c52a78e8'; // need to change contractBotABI in tools as well
Scenario.gasPriceIncrease = 1.3; //in percentage. Ex 1.2 will mean gas price is 120% fastest gas possible
Scenario.gasLimitDefault = 275000; //the default gas limit in computation units for a swap transaction
...
function setUpScenario() {
    if (args['help']) {
        ...
        console.log("--etherToSell=0.2                   Amount of eth to use to buy token");
    ...
    if (args['etherToSell']) {
        Scenario.etherToSell = args['etherToSell'];
    }
